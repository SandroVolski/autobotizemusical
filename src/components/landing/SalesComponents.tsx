...
export const MusicalTransition = () => {
  ...
  const NoteIcon = ({ type, className, style, refIdx }: { type: string; className: string; style: React.CSSProperties; refIdx: number }) => {
    ...
    return (
      <svg
        ref={el => { notesRef.current[refIdx] = el; }}
...
const mat4Utils = {
  create: () => {
    const out = new Float32Array(16); out[0] = out[5] = out[10] = out[15] = 1; return out;
  },
...
const quatUtils = {
  ...
  setAxisAngle: (out: Float32Array, axis: Float32Array, rad: number) => {
    rad = rad * 0.5; const s = Math.sin(rad);
...
class Geometry {
  ...
  subdivide(divisions = 1) {
    ...
    for (let div = 0; div < divisions; ++div) {
      ...
      this.faces.forEach(face => {
        const mAB = this.getMid(face.a, face.b, cache), mBC = this.getMid(face.b, face.c, cache), mCA = this.getMid(face.c, face.a, cache);
  ...
  getMid(ndxA: number, ndxB: number, cache: Record<string, number>) {
    ...
    const a = this.vertices[ndxA].position, b = this.vertices[ndxB].position, ndx = this.vertices.length;
    cache[key] = ndx;
...
class ArcballControl {
  ...
  proj(pos: Float32Array) {
    const w = this.canvas.clientWidth, h = this.canvas.clientHeight, s = Math.max(w, h);
    const x = (2 * pos[0] - w) / s, y = (2 * pos[1] - h) / s;
    const zSq = x * x + y * y;
...
export const SocialProofSphere = () => {
  ...
  const [activeItem, setActiveItem] = useState<{ initials: string; name: string; role: string; quote: string; color: string; image: string } | null>(null);
  const [isMoving, setIsMoving] = useState(false);

  const testimonials = [
    {
      initials: "SV", name: "Sandro Volski", role: "Proprietário da\nEscola de Música",
    ...
    {
      initials: "CS", name: "Carlos Santos", role: "Professor de Piano",
    ...
    {
      initials: "MS", name: "Maria Silva", role: "Proprietária do MusicFlow",
  ...
  useEffect(() => {
    ...
    const program = createProgram(gl, discVertShaderSource, discFragShaderSource, { aModelPosition: 0, aModelUvs: 2, aInstanceMatrix: 3 });
    if (!program) return;
    ...
    const drawAtlas = async (images: (HTMLImageElement | null)[] = []) => {
      ...
      for (let i = 0; i < testimonials.length; i++) {
        ...
        const x = (i % atlasSize) * cellSize, y = Math.floor(i / atlasSize) * cellSize, cx = x + 256, cy = y + 256;
        ctx.save();
...
export const NotebookFeaturesSection = () => {
  ...
  const features = [
    { title: "Gestão de Alunos", desc: "Registo completo, histórico de matrículas, progresso individual e comunicação direta.", img: "https://images.unsplash.com/photo-1577891776198-c28c302f0b1a?auto=format&fit=crop&w=800&q=80", bgText: "GESTÃO" },
    { title: "Agenda Inteligente", desc: "Agendamento automático, controlo de presença e sincronização com calendários externos.", img: "https://images.unsplash.com/photo-1506784365847-bbad939e9335?auto=format&fit=crop&w=800&q=80", bgText: "ESCOLA" },
    { title: "Controle Financeiro", desc: "Mensalidades, cobranças automáticas, relatórios de incumprimento e projeções.", img: "https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&w=800&q=80", bgText: "FINANÇAS" },
    { title: "IA Pedagógica", desc: "Sugestões de repertório, planos de aula personalizados e análise de evolução.", img: "https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=800&q=80", bgText: "INTELIGÊNCIA" },
    { title: "Material Didático", desc: "Biblioteca digital, pautas, vídeo-aulas e recursos partilhados.", img: "https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?auto=format&fit=crop&w=800&q=80", bgText: "MÚSICA" },
    { title: "Comunicação", desc: "Avisos automáticos, lembretes de aulas e notificações personalizadas.", img: "https://images.unsplash.com/photo-1577563906417-45a11b3f9f7c?auto=format&fit=crop&w=800&q=80", bgText: "CONEXÃO" },
    { title: "Relatórios Avançados", desc: "Dashboards em tempo real, métricas de desempenho e insights acionáveis.", img: "https://images.unsplash.com/photo-1551288049-bbbda536ad0a?auto=format&fit=crop&w=800&q=80", bgText: "DADOS" },
    { title: "Multi-instrumentos", desc: "Suporte para todos os instrumentos com configurações específicas para cada um.", img: "https://images.unsplash.com/photo-1511379938547-c1f69419868d?auto=format&fit=crop&w=800&q=80", bgText: "SISTEMA" }
  ];
  ...
  return (
    <div ref={notebookContainerRef} className="relative h-screen w-full overflow-hidden bg-zinc-950">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30 px-4">
        <div className="relative w-full max-w-[280px] sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl flex flex-col items-center" style={{ perspective: '2000px' }}>
          <div
            ref={lidRef}
      ...
      >
        {features.map((item, index) => (
          <div key={index} className="w-screen h-full flex flex-col items-center justify-between py-20 sm:py-28 flex-shrink-0 relative overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none z-0">
              <h4
                ...
                style={{ WebkitTextStroke: '1px rgba(255, 255, 255, 0.06)', opacity: activeIndex === index ? 0.8 : 0.1 }}
              >
                {item.bgText}
              </h4>
            </div>
      ...
      <div className="absolute bottom-8 left-0 w-full z-40 pointer-events-none">
        <div className="flex justify-center items-center gap-3">
          {features.map((_, i) => (
            <div key={i} className={`h-1.5 rounded-full transition-all duration-700 ${activeIndex === i ? 'w-16 bg-[#8000FF] shadow-[0_0_15px_rgba(128,0,255,0.4)]' : 'w-3 bg-zinc-800'}`}></div>
          ))}
        </div>
      </div>
    </div>
    ...
    <>
      ...
      <section ref={triggerRef} className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-0 flex items-center justify-center w-full h-full">
          <img
            ref={imageRef}
            src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJndXh3ZmxyeXJ3ZmxyeXJ3JmVwPXYxX2ludGVybmFs+Z2lmX2J5X2lkJmN0PWc/3o7TKMGpxxS06DclhS/giphy.gif"
            alt="Espiral"
...
export const PricingSection = () => {
  ...
  return (
    <section id="pricing-section" className="relative pt-0 pb-32 bg-black flex flex-col items-center montserrat-font overflow-hidden -mt-20 md:-mt-32">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black mb-10 tracking-tight uppercase leading-none text-white">
            Tudo o que sua escola <br /> precisa em um <span style={{ color: active.mainColor }} className="transition-colors duration-500">único lugar</span>
          </h2>
          <div className="flex items-center justify-center gap-4 mb-10">
            <span className={`text-sm font-bold uppercase tracking-widest transition-all duration-500 ${billingCycle === 'mensal' ? 'text-white' : 'text-white/40'}`}>Mensal</span>
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm font-bold uppercase tracking-widest transition-all duration-500 ${billingCycle === 'anual' ? 'text-white' : 'text-white/40'}`}>Anual</span>
        ...
        <div id="pricing-card" className="max-w-lg mx-auto relative group">
          <div className="absolute -inset-20 rounded-[5rem] blur-[110px] opacity-30 group-hover:opacity-100 transition-all duration-1000 animate-neon-pulse pointer-events-none"
            style={{ background: `linear-gradient(to bottom, ${active.mainColor}B3, ${active.contrastColor}33, transparent)` }}
          />
          <div className="relative bg-[#050505] border border-white/10 rounded-[2.5rem] p-10 md:p-14 shadow-2xl backdrop-blur-sm overflow-hidden text-center neon-border-hover">
            ...
            <div className="absolute -top-1 right-12 z-20">
              <div className="text-white text-[11px] font-black uppercase px-5 py-2.5 rounded-b-2xl flex items-center gap-1.5 shadow-xl transition-all duration-500"
            ...
            <div className="flex flex-col items-center justify-center mb-12 relative z-10 text-white">
              <div className="flex items-baseline gap-1">
                <span className="text-7xl md:text-8xl font-black tracking-tighter transition-all duration-500 group-hover:scale-105 inline-block"
                  style={{ color: active.mainColor }}>
                  R${active.price}
                </span>
                <span className="text-gray-500 font-bold text-lg">{active.period}</span>
              </div>
              <div className="mt-4 px-4 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all duration-500"
                style={{
            ...
            <button onClick={handleWhatsApp} onMouseEnter={() => setIsPricingHovered(true)} onMouseLeave={() => setIsPricingHovered(false)}
              ...
              <div className={`absolute inset-0 transition-colors duration-500 ${isPricingHovered ? 'bg-white' : ''}`} style={{ backgroundColor: isPricingHovered ? 'white' : active.mainColor }} />
              ...
              <div className="relative h-full w-full flex items-center justify-center">
                <span className={`absolute transition-all duration-500 uppercase tracking-widest ${isPricingHovered ? 'opacity-0 -translate-y-6' : 'opacity-100 translate-y-0 text-white'}`}>Começar agora</span>
                <span className={`absolute transition-all duration-500 flex items-center gap-3 uppercase tracking-widest ${isPricingHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
                  style={{ color: active.mainColor }}>
            ...
            <div className="mt-10 pt-8 border-t border-white/5 relative z-10 flex items-center justify-center gap-6 opacity-80 group-hover:opacity-100 transition-opacity">
              <span className="text-[11px] font-black uppercase tracking-widest transition-colors duration-500"
                style={{ color: active.contrastColor, textShadow: `0 0 10px ${active.contrastColor}80` }}>3 Dias Grátis</span>
            </div>
          </div>
        </div>
    ...
    <>
      ...
      >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-6">
              <div className="lg:col-span-6 space-y-12">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-[#8000FF] flex items-center justify-center font-black text-3xl italic rounded-2xl shadow-[0_0_40px_rgba(128,0,255,0.2)] text-white montserrat-font">AZ</div>
                  <div className="uppercase">
                    <h2 className="text-3xl font-black italic leading-none tracking-tighter text-white montserrat-font">Autobotize</h2>
                    <p className="text-[#8000FF] text-[10px] font-bold tracking-[0.6em] mt-1 montserrat-font uppercase">Gestão Empresarial</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-64 gap-y-10">
                  <div className="flex flex-col gap-3 items-start">
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 montserrat-font">E-mail</p>
              ...
              <div className="lg:col-span-6 grid grid-cols-2 gap-12 pt-4">
                <div className="space-y-8 text-white">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 montserrat-font">Seções</h4>
                ...
                <div className="space-y-8 text-white">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 montserrat-font">Social</h4>
                  <nav className="flex flex-col gap-2">
        ...
        <div className="w-full relative flex justify-center items-start h-8 max-w-[1600px] mx-auto">
          ...
          <div className="w-full max-w-[70%] h-8 bg-[#080808] mt-[-1px] relative z-0 overflow-hidden select-none" style={{ clipPath: 'url(#sub-footer-rhombus-rounded)' }} />
